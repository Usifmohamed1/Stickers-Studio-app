function CheckoutForm({ customer, setCustomer, totalQty, totalsByType, onBack, onSubmit, loading }) {
  ...
  return (
    <section>
      ...
      <p className="text-gray-600 text-sm text-center font-Chewy mb-8">
      ...
      <div className="relative flex items-center justify-center mb-10 mt-9">
        <div className="absolute left-0">
          <button
            onClick={onBack}
            className="px-3 py-1.5 rounded-2xl border-2 border-black [text-shadow:_1px_1px_0_#000,_2px_1px_0_#000] bg-sky-400 text-white text-2xl font-Chewy shadow-2xl active:scale-95 transition-transform "
          >
            Back
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-Chewy tracking-wide">
        ...
        {gov === "other" && (
          <label className="block">
            <div className="mb-1 text-sm">Governorate</div>
            <input
              value={customer.otherGov || ""}
              onChange={e => setCustomer({ ...customer, otherGov: e.target.value })}
              className="w-full px-3 py-2 rounded-xl border"
              placeholder="write your governorate"
            />
          </label>
        )}
      ...
      {/* Totals */}
      <div className="text-lg text-center text-gray-700 mt-10 mb-2 font-Chewy">
        Total quantity <b className="text-sky-400 text-2xl">{totalQty}</b>
      </div>
      <div className="text-lg text-center text-gray-700 font-Chewy">
        Posters <b className="text-sky-400 text-2xl">{totalsByType.posters}</b>
      </div>
      <div className="text-lg text-center text-gray-700 font-Chewy">
        Single <b className="text-sky-400 text-2xl">{totalsByType.single}</b>
      </div>
      <div className="text-lg text-center text-gray-700 font-Chewy">
        Double <b className="text-sky-400 text-2xl">{totalsByType.double}</b>
      </div>
      ...
      <div className="mt-6 flex justify-end">
        <button
          disabled={loading}
          onClick={onSubmit}
          className="px-4 py-2 rounded-2xl bg-sky-400 text-white font-Chewy text-lg tracking-wide [text-shadow:_1px_1px_0_#000,_2px_1px_0_#000] border-2 border-black"
        >
          {loading ? (
