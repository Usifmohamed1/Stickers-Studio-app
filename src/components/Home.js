function CategoryCard({ title, onClick, img }) {
  return (
  <button onClick={onClick} className="group relative overflow-hidden rounded-2xl shadow-2xl aspect-[12.7/16] text-left active:scale-95 transition-transform">
      <img src={img} alt={title} className="absolute inset-0 w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
  ...
  return (
    <section>
      <div className="flex flex-row items-center justify-between md:justify-center mb-4 w-full">
        ...
        {/* Show the cart icon button only if there are items in the cart (mobile view) */}
        {totalQty > 0 && (
          <button
            onClick={onGoCart}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-14 mb-10 mt-5 active:scale-95 transition-transform">
        <CategoryCard onClick={() => gotoCategory("stickers")} img="https://i.postimg.cc/bNW9QV3n/stickers.png" />
        <CategoryCard onClick={() => gotoCategory("posters")} img="https://i.postimg.cc/nVYLKX2G/pos-358-x-452-px.png" />
      </div>
    </section>
  );
}
